...
#     plt.imshow(mask, cmap='viridis') 
    ...
    tensor_image = tensor_image[0].view(1,784)
    ...
    with torch.no_grad():
        ...
        print(certainty, output)
        certainty = certainty.clone().cpu().item()
        output = output.clone().cpu().item()
        certainty1, output1 = torch.topk(output0[0],3)
        certainty1 = certainty1.clone().cpu()#.item()
        output1 = output1.clone().cpu()#.item()
    ps = torch.exp(output0)
    prob = list(ps.numpy()[0])
    probab = prob.index(max(prob))
#     print(certainty)
    st.write('### Prediction') 
    st.write('### '+str(probab))
    st.write('### First layer weight visualisation')
    st.image('firstweightplot.png')
